...
public class DataStore {
	...
	static {
		...
		Product roadster_t_shirt = new Product(1, "880589", "Roadster T-Shirt",
				"Black, V-Neck, Long sleeves", 499);
		Product harley_t_shirt = new Product(
				2,
				"1292788",
		...
		Product uspolo_t_shirt = new Product(
				3,
				"427414",
		...
		Product caprese_wallet = new Product(
				8,
				"1247569",
		...
		Product titan_watch = new Product(
				9,
				"1399728",
		...
		Order order1 = new Order(1, "124719220", Date.valueOf("2016-01-26"),
				ophelia, OrderStatus.ACTIVE, new HashSet<Product>(
						Arrays.asList(adidas_basketball, uspolo_t_shirt)));
		Order order2 = new Order(2, "119215410", Date.valueOf("2016-02-14"),
				gertude, OrderStatus.ACTIVE, new HashSet<Product>(
						Arrays.asList(titan_watch, caprese_wallet,
								harley_t_shirt)));
		Order order3 = new Order(3, "119215340", Date.valueOf("2016-03-23"),
				laertes, OrderStatus.ACTIVE, new HashSet<Product>(
						Arrays.asList(caprese_wallet, stevemadden_handbag,
								jackjones_t_shirt, adidas_flipflops)));
		Order order4 = new Order(4, "116410240", Date.valueOf("2016-04-01"),
				fortinbras, OrderStatus.ACTIVE, new HashSet<Product>(
						Arrays.asList(uspolo_t_shirt, stevemadden_handbag)));
		Order order5 = new Order(5, "96357758", Date.valueOf("2016-05-01"),
				gertude, OrderStatus.ACTIVE, new HashSet<Product>(
						Arrays.asList(caprese_wallet)));
		Order order6 = new Order(6, "80472941", Date.valueOf("2016-08-15"),
				fortinbras, OrderStatus.ACTIVE, new HashSet<Product>(
						Arrays.asList(jackjones_t_shirt, adidas_flipflops,
								titan_watch)));
		...
		Wishlist wishlist1 = new Wishlist(1, ophelia, new HashSet<Product>(
				Arrays.asList(caprese_wallet, roadster_t_shirt)));
		Wishlist wishlist2 = new Wishlist(2, gertude,
				new HashSet<Product>(Arrays.asList(titan_watch,
						adidas_basketball, adidas_flipflops)));
		Wishlist wishlist3 = new Wishlist(3, laertes, new HashSet<Product>(
				Arrays.asList(jackjones_t_shirt)));
	...
	public User getUser(String username) throws UserNotFoundException {
		...
		throw new UserNotFoundException(new ExceptionMessage(String.format(
	...
	public Product getProduct(String productCode)
		...
		throw new ProductNotFoundException(new ExceptionMessage(String.format(
				"Product with code %s not found", productCode)));
	}

	public Order getOrderForUser(String username, String orderNumber)
			throws OrderNotFoundException {
		for (Order order : orders) {
			if (order.getNumber().equals(orderNumber)
					&& order.getOrderedBy().getUsername().equals(username))
				return order;
		}
		throw new OrderNotFoundException(new ExceptionMessage(String.format(
				"Order %s :: not found for user %s", orderNumber, username)));
	}
	...
	public Wishlist getWishlistForUser(String username)
		...
		for (Wishlist wishlist : wishlists) {
			if (wishlist.getWishedBy().getUsername().equals(username))
				return wishlist;
		}
		throw new WishlistNotFoundException(new ExceptionMessage(String.format(
				"No wishlist yet for '%s'", username)));
	}
	...
	public void deleteUser(User userToDelete) {
		for (int i = 0; i < users.size(); i++)
			if (users.get(i).getUserId() == userToDelete.getUserId())
				users.remove(i);
	}
	...
	public void updateOrder(Order newOrder) {
		for (Order order : orders) {
			if (order.getOrderId() == newOrder.getOrderId()) {
				...
				order.setOrderedOn(newOrder.getOrderedOn());
